...
colnames(data_list)<-c("Parameter","Symbol in R",
                    "Initial values")
knitr::kable(data_list, caption = "Table 1: Parameters used in our model as in R")
```
...
After setting up all the componenets of our model, we can build the demographic process using the function `build.demographic.process`. This output can be used as imput to coalescent models for the calculation of the likelihood, when fitting the model using a Markov chain Monte Carlo (MCMC), for example.
