...
```{r out.width = "60%", fig.align="center", fig.cap="**Figure 1.** Transmission model for HIV in Senegal. $gpf$, $gpm$ and $msm$ represent infected individuals.", echo = FALSE}
...
Finally, to model the HIV epidemic in Senegal, we also added an additional compartment named "source" ($src$), that represents the rate in which HIV lineages are imported to Senegal from other countries. We modelled this as a constant efective population size rate with two parameters to be estimeted -- $srcNe$: the effective source population size; and the $import$ rate. Because these are modeled as constant effective population size, the infected $src$ individuals along time are not represented in the ODEs. 
...
kable_table <- knitr::kable(data_list, caption = "**Table 1.** Parameter definition, values and symbols used in the R script")
...
kable_table <- knitr::kable(births, caption = "**Table 2.** Illustration of allowed transmissions between demes.")
...
kable_table <- knitr::kable(migs, caption = "**Table 3.** Illustration of allowed migrations between demes.")
...
# metadata from children, or risk group = NA, or sex = NA will also be removed. 
all_data <- senegalHIVmodel::organize_metadata(all.data.cgr, all.data.SN, tree.all)
```
...
densities <-  function(par){
  # d1 to d3 and d5 to d7 I am using a gamma distribution with mean = R0 = 1.1 and sigma = 1
  # d4 and d8 uniform distribution between the start time and the final time of our simualtions (t0 = 1978, and t1 = 2014)
