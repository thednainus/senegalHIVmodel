...
tip2isGPM <- function(tip)
{
  if ( ! grepl('SN' , tip )) return(FALSE)
  state  = all_data$States[ all_data$tip.name == tip ]
  if ( state == "gpm" ) return(TRUE)
  return(FALSE)
}
...
gpm_seq <- sapply(dated.tree$tip.label, tip2isGPM)
...
tree.noGPM <- drop.tip(tree.dakar, dated.tree$tip.label[gpm_seq])
dated.tree.dakar.noGPM <- DatedTree(phylo = tree.noGPM,
                              sampleTimes = times[tree.noGPM$tip.label],
                              sampleStates = sampleStates[tree.noGPM$tip.label, ],
                              minEdgeLength = 2/52,
                              tol = 0.1)
