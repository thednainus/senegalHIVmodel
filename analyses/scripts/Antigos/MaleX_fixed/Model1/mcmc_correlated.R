...
# Here I wil also tryto propose together correlated parameters using the
# k and h parameters (see ?DEZa in the Bayesian Tools package)
bayesianSetup <- createBayesianSetup(likelihood = obj_fun , prior = prior)
load("startValue.rda")

settings = list(iterations = 6000, nrChains = 1, thin = 1, startValue = startValue,
                blockUpdate = list("correlation", h = 0.80, pSel = NULL,
                                   pGroup = NULL, groupStart = 1000,
                                   groupIntervall = 500))
...
runZ <- readRDS("correlated500.rds")
...
saveRDS(outZ, "withZmatrixCorrelated.RDS")
