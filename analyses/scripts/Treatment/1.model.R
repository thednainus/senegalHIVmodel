# Model including compartment for treatment
...
demes <- c('gpm', 'gpf', 'msm', "Tgpm", "Tgpf", "Tmsm", 'src')
...
THETA <- list(
  ...
  pmsm2msm = 0.85,
  pgpf2gpm = 0.85,
  delta = 0.035, #death rate while at ART
  sigma = 0.03,
  initmsm = 1, # Loss to follow up from ART per year
  initgp = 1,
  initTgp = 0,
  initTmsm = 0
)
...
SRCSIZE <<- 1e5
# X0 is the initial conditions for the 6 demes (gpf, gpm, msm, Tgpf, Tgpm, Tmsm, src)
X0 <- c(gpm = unname(THETA$initgp/2),
        gpf = unname(THETA$initgp/2),
        msm = unname(THETA$initmsm),
        Tgpm = unname(THETA$initTgp/2),
        Tgpf = unname(THETA$initTgp/2),
        Tmsm = unname(THETA$initTmsm),
        src = SRCSIZE)

# Because there are 7 demes in this model, the birth matrix is a 7 x 7 matrix
...
# Migrations is also a 7 x 7 matrix because we have 7 demes
...
migs['gpm', 'Tgpm'] <- 'omega * gpm'
migs['gpf', 'Tgpf'] <- 'omega * gpf'
migs['msm', 'Tmsm'] <- 'omega * msm'

migs['Tgpm', 'gpm'] <- 'sigma * Tgpm'
migs['Tgpf', 'gpf'] <- 'sigma * Tgpf'
migs['Tmsm', 'msm'] <- 'sigma * Tmsm'

# Deaths is a vector that showed in which rate a lineage dies
deaths['msm'] <- 'GAMMA * msm'
deaths['gpf'] <- 'GAMMA * gpf'
deaths['gpm'] <- 'GAMMA * gpm'
deaths['Tgpm'] <- 'delta * Tgpm'
deaths['Tgpf'] <- 'delta * Tgpf'
deaths['Tmsm'] <- 'delta * Tmsm'
