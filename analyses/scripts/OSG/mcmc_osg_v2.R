...
value1 <-  fits.ordered[[pos1]]$par
value2 <-  fits.ordered[[pos2]]$par
value3 <-  fits.ordered[[pos3]]$par
...
while(i < 61){
  ...
  if(!file.exists("out.RDS")){
    settings = list(iterations = 100, startValue =  initialValues, nrChains = 1, thin = 1)
