#Function to read csv file containing metadata and format it a way to be used as
#input data for tredater
library(treedater)
library(lubridate)
library(phytools)

###### FUNCTIONS ##########
#Data frame manipulations to run treedater
#hiv.df = dataframe containg metadata
#subtype = character with the name of subtype in the form of B.SN or B.SN.CGR, for example
#country = should be "SN" or "SN.CGR"
data_format <- function(hiv.df, country){
  if(country != "CGR"){
    hiv.df$tip <- paste(hiv.df$Accession_number, hiv.df$Subtype, country, hiv.df$Year, sep='.')
  }else{hiv.df$tip <- paste(hiv.df$Accession_number, country, sep='_')}

  hiv.df$date <- paste(hiv.df$Year, hiv.df$Month, hiv.df$Day, sep='-')
  hiv.df$date_ymd <- ymd(hiv.df$date)
  hiv.df$decimal <- decimal_date(hiv.df$date_ymd)
  hiv.df$decimal <- as.factor(hiv.df$decimal)

  missing_sample <- hiv.df[is.na(hiv.df$decimal),]
  str(missing_sample)
  missing_sample$lower_date <- paste(missing_sample$Lower_year, missing_sample$Lower_month, missing_sample$Lower_day, sep='-')
  missing_sample$lower_ymd <- ymd(missing_sample$lower_date)
  missing_sample$lower <- decimal_date(missing_sample$lower_ymd)

  missing_sample$upper_date <- paste(missing_sample$Upper_year, missing_sample$Upper_month, missing_sample$Upper_day, sep='-')
  missing_sample$upper_ymd <- ymd(missing_sample$upper_date)
  missing_sample$upper <- decimal_date(missing_sample$upper_ymd)

  return(list(hiv.df, missing_sample))
}

# Plot lineage  through time with confidence interval (below is the function plot.parboot.ltt) using parametric bootstrap
parboot.ltt <- function(pbtd)
{
  ...
  ltt <- cbind(times = times, t(sapply(times, function(t) {
    c(pml = sum(pbtd$td$sts > t) - sum(pbtd$td$Ti > t), setNames(quantile(sapply(pbtd$trees, function(tre) sum(tre$sts > t) - sum(tre$Ti > t)),
...
# Function to re-root bootstrap trees and drop outgroup tips (after re-rooting trees)
root.and.drop.tips <- function(tree, outgroups){
...
setwd("~/Box Sync/Senegal/HIV_myDesktop/data/")
#setwd("~/Desktop/Box_backup/HIV_myDesktop/data")
...
B.missing_sample.df <- rbind(B.missing_sample[c("tip","lower","upper")], B.CGR.missing_sample[c("tip","lower","upper")])
B.missing_sample.df.2 <- B.missing_sample.df[(c("lower","upper"))]
row.names(B.missing_sample.df.2) <- B.missing_sample.df[,1]



# chech which sequence is not present in the phylogeny but present in the database
B.df <- rbind(B.SN.df[c("Accession_number","tip","decimal")], B.CGR.df[c("Accession_number","tip","decimal")])
#B.df[!B.df$tip %in% tB.CGR.GTR_G$tip.label,]
#B.df2 <- subset(B.df, Accession_number != "AJ583740" & Accession_number != "FN599677" & Accession_number != "LN680713")
...
(tB.CGR.GTR_G <- read.tree("~/Box Sync/HIV_myDesktop/data/B/additional_data/raxml-ng/no_duplicated/CGR/by_codon/B_CGR_GTR_G/ML_0_GTR+G.raxml.support.tre"))
quartz()
plot(tB.CGR.GTR_G)
nodelabels()
(tB.CGR.GTR_G.r <- reroot(tB.CGR.GTR_G, node.number=84, edgelabel = TRUE))
plot(tB.CGR.GTR_G.r)
(tB.CGR.GTR_G.r.new <- drop.tip(tB.CGR.GTR_G.r, tip=c("Ref.D.CM.01.01CM_4412HAL.AY371157", "Ref.D.CD.83.ELI.K03454", "Ref.D.TZ.01.A280.AY253311")))
...
(tB.CGR.GTR_Gp <- read.tree("~/Box Sync/HIV_myDesktop/data/B/additional_data/raxml-ng/no_duplicated/CGR/by_codon/B_CGR_GTR_Gp/ML_0_GTR+Gp.raxml.support.tre"))
quartz()
plot(tB.CGR.GTR_Gp)
nodelabels()
(tB.CGR.GTR_Gp.r <- reroot(tB.CGR.GTR_Gp, node.number=95, edgelabel = TRUE))
plot(tB.CGR.GTR_Gp.r)
(tB.CGR.GTR_Gp.r.new <- drop.tip(tB.CGR.GTR_Gp.r, tip=c("Ref.D.CM.01.01CM_4412HAL.AY371157", "Ref.D.CD.83.ELI.K03454", "Ref.D.TZ.01.A280.AY253311")))
plot(ladderize(tB.CGR.GTR_Gp.r.new, right = FALSE), show.node.label = TRUE)



# DNA substitution model: GTR+R3
(tB.CGR.GTR_R3 <- read.tree("~/Box Sync/HIV_myDesktop/data/B/additional_data/raxml-ng/no_duplicated/CGR/by_codon/B_CGR_GTR_R/ML_0_GTR+R3.raxml.support.tre"))
quartz()
plot(tB.CGR.GTR_R3)
nodelabels()
...
(tB.CGR.GTR_I_G <- read.tree("~/Box Sync/HIV_myDesktop/data/B/additional_data/raxml-ng/no_duplicated/CGR/by_codon/B_CGR_GTR_I_G/ML_0_CGR_GTR+I+G.raxml.support.tre"))
quartz()
plot(tB.CGR.GTR_I_G)
nodelabels()
...
(dtr.B.GTR_G <- dater(tB.CGR.GTR_G.r.new, B.sts, B.seqlen, estimateSampleTimes = B.missing_sample.df.2))
(dtr.B.GTR_Gp <- dater(tB.CGR.GTR_Gp.r.new, B.sts, B.seqlen, estimateSampleTimes = B.missing_sample.df.2))
(dtr.B.GTR_I_G <- dater(tB.CGR.GTR_I_G.r.new, B.sts, B.seqlen, estimateSampleTimes = B.missing_sample.df.2))
(dtr.B.GTR_R3 <- dater(tB.CGR.GTR_R3.r.new, B.sts, B.seqlen, estimateSampleTimes = B.missing_sample.df.2))
...
# GTR+G
B.GTR.G <- data.frame(times=po_dtr.B.CGR.GTR_G$Ti, Bootstrap = po_B_CGR.GTR_G$node.label)
B.GTR.G$Bootstrap <- as.character(B.GTR.G$Bootstrap)
B.GTR.G$Bootstrap <- as.numeric(B.GTR.G$Bootstrap)

# GTR+R3
B.GTR.R3 <- data.frame(times=po_dtr.B.CGR.GTR_R3$Ti, Bootstrap = po_B_CGR.GTR_R3$node.label)
...
#boot.B.GTR_G <- parboot.treedater(dtr.B.GTR_G)
#boot.B.GTR_R3 <- parboot.treedater(dtr.B.GTR_R3)


#boot.B.GTR_G.ltt <- parboot.ltt(boot.B.GTR_G)
#boot.B.GTR_G.ltt$group = "B.CGR.GTR+G"
#boot.B.GTR_R3.ltt <- parboot.ltt(boot.B.GTR_R3)
...
#quartz()
#p <- ggplot(boot.B.comb.ltt, aes(color = group, group = group)) + geom_ribbon(aes(x = times, ymin = lb, ymax = ub, fill = group, col = group), alpha = 0.1)
#p <- p + geom_path(aes(x = times, y = pml)) + scale_fill_manual(values=c("#0072B2", "#D55E00")) + scale_colour_manual(values=c("#0072B2", "#D55E00")) + theme_bw()
...
B.CGR.GTR_G_100boot <- read.tree("~/Box Sync/HIV_myDesktop/data/B/additional_data/raxml-ng/no_duplicated/CGR/by_codon/B_CGR_GTR_G/ML_0_GTR+G.raxml.bootstraps.tre")[1:100]
B.CGR.GTR_G_100boot.r.new <- lapply(unclass(B.CGR.GTR_G_100boot), root.and.drop.tips, outgroups=c("Ref.D.CM.01.01CM_4412HAL.AY371157", "Ref.D.CD.83.ELI.K03454", "Ref.D.TZ.01.A280.AY253311"))
class(B.CGR.GTR_G_100boot.r.new)<-"multiPhylo"

B.CGR.GTR_R3_100boot <- read.tree("~/Box Sync/HIV_myDesktop/data/B/additional_data/raxml-ng/no_duplicated/CGR/by_codon/B_CGR_GTR_R/ML_0_GTR+R3.raxml.bootstraps.tre")[1:100]
B.CGR.GTR_R3_100boot.r.new <- lapply(unclass(B.CGR.GTR_R3_100boot), root.and.drop.tips, outgroups=c("Ref.D.CM.01.01CM_4412HAL.AY371157", "Ref.D.CD.83.ELI.K03454", "Ref.D.TZ.01.A280.AY253311"))
class(B.CGR.GTR_R3_100boot.r.new)<-"multiPhylo"

B.CGR.GTR_Gp_100boot <- read.tree("~/Box Sync/HIV_myDesktop/data/B/additional_data/raxml-ng/no_duplicated/CGR/by_codon/B_CGR_GTR_Gp/ML_0_GTR+Gp.raxml.bootstraps.tre")[1:100]
B.CGR.GTR_Gp_100boot.r.new <- lapply(unclass(B.CGR.GTR_Gp_100boot), root.and.drop.tips, outgroups=c("Ref.D.CM.01.01CM_4412HAL.AY371157", "Ref.D.CD.83.ELI.K03454", "Ref.D.TZ.01.A280.AY253311"))
...
#Dataframe to be used in treedater for the missing sample date
C.missing_sample.df <- rbind(C.missing_sample[c("tip", "lower", "upper")], C.CGR.missing_sample[c("tip", "lower", "upper")])
...
C.df <- rbind(C.SN.df[c("Accession_number","tip","decimal")], C.CGR.df[c("Accession_number","tip","decimal")])
...
plot(tC.CGR.GTR_G.r)
tC.CGR.GTR_G.r.new <- drop.tip(tC.CGR.GTR_G.r, tip=c("Ref.D.CM.01.01CM_4412HAL.AY371157", "Ref.D.CD.83.ELI.K03454", "Ref.D.TZ.01.A280.AY253311"))
...
tC.CGR.GTR_Gp.r.new <- drop.tip(tC.CGR.GTR_Gp.r, tip=c("Ref.D.CM.01.01CM_4412HAL.AY371157", "Ref.D.CD.83.ELI.K03454", "Ref.D.TZ.01.A280.AY253311"))
...
(tC.CGR.GTR_I_G.r.new <- drop.tip(tC.CGR.GTR_I_G.r, tip=c("Ref.D.CM.01.01CM_4412HAL.AY371157", "Ref.D.CD.83.ELI.K03454", "Ref.D.TZ.01.A280.AY253311")))
...
plot(tC.CGR.GTR_R.r)
tC.CGR.GTR_R.r.new <- drop.tip(tC.CGR.GTR_R.r, tip=c("Ref.D.CM.01.01CM_4412HAL.AY371157", "Ref.D.CD.83.ELI.K03454", "Ref.D.TZ.01.A280.AY253311"))
...
(dtr.C.GTR_G <- dater(tC.CGR.GTR_G.r.new, C.sts, C.seqlen, estimateSampleTimes = C.missing_sample.df.2))
(dtr.C.GTR_Gp <- dater(tC.CGR.GTR_Gp.r.new, C.sts, C.seqlen, estimateSampleTimes = C.missing_sample.df.2))
(dtr.C.GTR_I_G <- dater(tC.CGR.GTR_I_G.r.new, C.sts, C.seqlen, estimateSampleTimes = C.missing_sample.df.2))
(dtr.C.GTR_R <- dater(tC.CGR.GTR_R.r.new, C.sts, C.seqlen, estimateSampleTimes = C.missing_sample.df.2))
...
# GTR+G
C.GTR.G <- data.frame(times=po_dtr.C.CGR.GTR_G$Ti, Bootstrap = po_C_CGR.GTR_G$node.label)
C.GTR.G$Bootstrap <- as.character(C.GTR.G$Bootstrap)
C.GTR.G$Bootstrap <- as.numeric(C.GTR.G$Bootstrap)

# GTR+R
C.GTR.R <- data.frame(times=po_dtr.C.CGR.GTR_R$Ti, Bootstrap = po_C_CGR.GTR_R$node.label)
...
p <- ggplot(boot.C.comb.ltt, aes(color = group, group = group)) + geom_ribbon(aes(x = times, ymin = lb, ymax = ub, fill = group, col = group), alpha = 0.1)
p <- p + geom_path(aes(x = times, y = pml)) + scale_fill_manual(values=c("#0072B2", "#D55E00")) + scale_colour_manual(values=c("#0072B2", "#D55E00")) + theme_bw()
(p <- p + ylab("Lineages through time") + xlab("Time"))
...
# Rooting bootstrap trees using the outgroups and removing outgroups from phylogenetic trees
C.CGR.GTR_G_100boot <- read.tree(c)[1:100]
C.CGR.GTR_G_100boot.r.new <- lapply(unclass(C.CGR.GTR_G_100boot), root.and.drop.tips, outgroups=c("Ref.D.CM.01.01CM_4412HAL.AY371157", "Ref.D.CD.83.ELI.K03454", "Ref.D.TZ.01.A280.AY253311"))
class(C.CGR.GTR_G_100boot.r.new)<-"multiPhylo"

C.CGR.GTR_R_100boot <- read.tree("~/Box Sync/HIV_myDesktop/data/C/additional_data/raxml-ng/no_duplicated/CGR/C_CGR_GTR+R/final.BOOT.C.CGR.GTR+R.raxml.bootstraps.tre")[1:100]
C.CGR.GTR_R_100boot.r.new <- lapply(unclass(C.CGR.GTR_R_100boot), root.and.drop.tips, outgroups=c("Ref.D.CM.01.01CM_4412HAL.AY371157", "Ref.D.CD.83.ELI.K03454", "Ref.D.TZ.01.A280.AY253311"))
...
#Dataframe to be used in treedater for the missing sample date
AG.missing_sample.df <- rbind(AG.missing_sample[c("tip","lower","upper")], AG.CGR.missing_sample[c("tip","lower","upper")])
AG.missing_sample.df.2 <- AG.missing_sample.df[(c("lower","upper"))]
row.names(AG.missing_sample.df.2) <- AG.missing_sample.df[,1]

AG.df <- rbind(AG.SN.df[c("Accession_number","tip","decimal")], AG.CGR.df[c("Accession_number","tip","decimal")])
#(tAG.CGR.GTR_G <- read.tree("~/Box Sync/HIV_myDesktop/data/02_AG/additional_data/raxml-ng/no_duplicated/CGR/02_AG_CGR_GTR+G/final.ML_13_GTR+G.raxml.bestTree.raxml.support.tre"))
#AG <- AG.df[!AG.df$tip %in% tAG.CGR.GTR_G$tip.label,]

#AG.df2 <- subset(AG.df, Accession_number != ????)
...
(tAG.CGR.GTR_G.r.new <- drop.tip(tAG.CGR.GTR_G.r, tip=c("Ref.A1.UG.92.92UG037.AB253429", "Ref.A1.AU.03.PS1044_Day0.DQ676872", "Ref.A1.RW.92.92RW008.AB253421")))
...
(tAG.CGR.GTR_Gp.r.new <- drop.tip(tAG.CGR.GTR_Gp.r, tip=c("Ref.A1.UG.92.92UG037.AB253429", "Ref.A1.AU.03.PS1044_Day0.DQ676872", "Ref.A1.RW.92.92RW008.AB253421")))
...
(tAG.CGR.GTR_I_G.r.new <- drop.tip(tAG.CGR.GTR_I_G.r, tip=c("Ref.A1.UG.92.92UG037.AB253429", "Ref.A1.AU.03.PS1044_Day0.DQ676872", "Ref.A1.RW.92.92RW008.AB253421")))
...
(tAG.CGR.GTR_R.r.new <- drop.tip(tAG.CGR.GTR_R.r, tip=c("Ref.A1.UG.92.92UG037.AB253429", "Ref.A1.AU.03.PS1044_Day0.DQ676872", "Ref.A1.RW.92.92RW008.AB253421")))
...
(dtr.AG.CGR.GTR_G <- dater(tAG.CGR.GTR_G.r.new, AG.sts, AG.seqlen, estimateSampleTimes = AG.missing_sample.df.2) )
(dtr.AG.CGR.GTR_Gp <- dater(tAG.CGR.GTR_Gp.r.new, AG.sts, AG.seqlen, estimateSampleTimes = AG.missing_sample.df.2) )
(dtr.AG.CGR.GTR_I_G <- dater(tAG.CGR.GTR_I_G.r.new, AG.sts, AG.seqlen, estimateSampleTimes = AG.missing_sample.df.2) )
(dtr.AG.CGR.GTR_R <- dater(tAG.CGR.GTR_R.r.new, AG.sts, AG.seqlen, estimateSampleTimes = AG.missing_sample.df.2) )
...
p <- ggplot(boot.AG.comb.ltt, aes(color = group, group = group)) + geom_ribbon(aes(x = times, ymin = lb, ymax = ub, fill = group, col = group), alpha = 0.1)
p <- p + geom_path(aes(x = times, y = pml)) + scale_fill_manual(values=c("#0072B2", "#D55E00")) + scale_colour_manual(values=c("#0072B2", "#D55E00")) + theme_bw()
(p <- p + ylab("Lineages through time") + xlab("Time"))
...
#### non-parametric bootstrap for treedater trees #######
# Rooting bootstrap trees using the outgroups and removing outgroups from phylogenetic trees
AG.CGR.GTR_G_100boot <- read.tree("~/Box Sync/HIV_myDesktop/data/02_AG/additional_data/raxml-ng/no_duplicated/CGR/by_codon/02_AG_CGR_GTR+G/final.BOOT.02_AG.CGR.GTR+G.raxml.bootstraps.tre")[1:2]
AG.CGR.GTR_G_100boot.r.new <- lapply(unclass(AG.CGR.GTR_G_100boot), root.and.drop.tips, outgroups=c("Ref.A1.UG.92.92UG037.AB253429", "Ref.A1.AU.03.PS1044_Day0.DQ676872", "Ref.A1.RW.92.92RW008.AB253421"))
class(AG.CGR.GTR_G_100boot.r.new)<-"multiPhylo"


AG.CGR.GTR_R_100boot <- read.tree("~/Box Sync/HIV_myDesktop/data/02_AG/additional_data/raxml-ng/no_duplicated/CGR/02_AG_CGR_GTR+R/final.BOOT.02_AG.CGR.GTR+R.raxml.bootstraps.tre")[1:100]
AG.CGR.GTR_R_100boot.r.new <- lapply(unclass(AG.CGR.GTR_R_100boot), root.and.drop.tips, outgroups=c("Ref.A1.UG.92.92UG037.AB253429", "Ref.A1.AU.03.PS1044_Day0.DQ676872", "Ref.A1.RW.92.92RW008.AB253421"))
