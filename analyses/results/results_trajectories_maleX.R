...
############################## Model 2 ########################################
m2.s <- getSample(m2, start=1000)
m2_map <- MAP(m2, start=1000)$parametersMAP
...
m2_o.1000 <- do.call("cbind", apply(m2.1000, 1, post_traj_mx, THETA))
# solve the demographic model for MAP (maximum a posteriori)
m2_map_o <- post_traj_mx(parameters = m2_map, THETA = THETA)


############################## Model 3 ########################################
m3.s <- getSample(m3, start=1000)
m3_map <- MAP(m3, start=1000)$parametersMAP
...
# solve the demographic model for 100 combination of parameter values
#m3_o <- do.call("cbind", apply(m3.100, 1, post_traj_mx, THETA))
# solve the demographic model for 1000 combination of parameter values
m3_o.1000 <- do.call("cbind", apply(m3.1000, 1, post_traj_mx, THETA))
# solve the demographic model for MAP
m3_map_o <- post_traj_mx(parameters = m3_map, THETA = THETA)


############################## Model 4 ########################################
m4.s <- getSample(m4, start=1200)
m4_map <- MAP(m4, start=1200)$parametersMAP
...
# solve the demographic model for 100 combination of parameter values
#m4_o <- do.call("cbind", apply(m4.100, 1, post_traj_mx, THETA))
# solve the demographic model for 1000 combination of parameter values
m4_o.1000 <- do.call("cbind", apply(m4.1000, 1, post_traj_mx, THETA))
# solve the demographic model for MAP
m4_map_o <- post_traj_mx(parameters = m4_map, THETA = THETA)
...
# solve the demographic model for 100 combination of parameter values
#m02_AG_m1_o <- do.call("cbind", apply(m02_AG.m1.100, 1, post_traj_mx, THETA))
# solve the demographic model for 1000 combination of parameter values
m02_AG_m1_o.1000 <- do.call("cbind", apply(m02_AG.m1.1000, 1, post_traj_mx, THETA))
# solve the demographic model for MAP
m02_AG_m1_map_o <- post_traj_mx(parameters = m02_AG.m1_map, THETA = THETA)


######################### by subtype 02_AG: Model 2 ###########################
m02_AG.m2.s <- getSample(m2.m02_AG.mx.m_rd2, start=1500)
m02_AG.m2_map <- MAP(m2.m02_AG.mx.m_rd2, start=1500)$parametersMAP
...
# solve the demographic model for 100 combination of parameter values
#m02_AG_m2_o <- do.call("cbind", apply(m02_AG.m2.100, 1, post_traj_mx, THETA))
# solve the demographic model for 1000 combination of parameter values
m02_AG_m2_o.1000 <- do.call("cbind", apply(m02_AG.m2.1000, 1, post_traj_mx, THETA))
# solve the demographic model for MAP
m02_AG_m2_map_o <- post_traj_mx(parameters = m02_AG.m2_map, THETA = THETA)



######################### by subtype C: Model 1 ##############################
mC.m1.s <- getSample(C_m1.m, start=1000)
mC.m1_map <- MAP(C_m1.m, start=1000)$parametersMAP
...
# solve the demographic model for 100 combination of parameter values
#C_m1_o <- do.call("cbind", apply(mC.m1.100, 1, post_traj_mx, THETA))
# solve the demographic model for 1000 combination of parameter values
C_m1_o.1000 <- do.call("cbind", apply(mC.m1.1000, 1, post_traj_mx, THETA))
# solve the demographic model for MAP
mC_m1_map_o <- post_traj_mx(parameters = mC.m1_map, THETA = THETA)



######################### by subtype C: Model 2 ################################
mC.m2.s <- getSample(C_m2.m, start=1000)
mC.m2_map <- MAP(C_m2.m, start=1000)$parametersMAP
...
# solve the demographic model for 100 combination of parameter values
#C_m2_o <- do.call("cbind", apply(mC.m2.100, 1, post_traj_mx, THETA))
# solve the demographic model for 1000 combination of parameter values
C_m2_o.1000 <- do.call("cbind", apply(mC.m2.1000, 1, post_traj_mx, THETA))
# solve the demographic model for MAP
mC_m2_map_o <- post_traj_mx(parameters = mC.m2_map, THETA = THETA)
...
save(m2_o.1000, m4_o.1000, C_m1_o.1000,
     file="analyses/plots/solved_objects_1000reps_new.rda")
save(m2_map_o, m4_map_o, mC_m1_map_o,
     file="analyses/plots/solved_objects_maps_new.rda")
